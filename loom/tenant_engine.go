...
// Get retrieves a task by ID only if it belongs to this tenant and engine.
// Returns ErrTaskNotFound when the task does not exist, belongs to a different
// tenant, or belongs to a different engine (404 semantics, no disclosure).
func (t *TenantScopedLoomEngine) Get(taskID string) (*Task, error) {
	return t.GetContext(context.Background(), taskID)
}

// GetContext retrieves an engine- and tenant-scoped task with caller-controlled cancellation.
func (t *TenantScopedLoomEngine) GetContext(ctx context.Context, taskID string) (*Task, error) {
	return t.engine.store.GetForTenantInEngineContext(ctx, taskID, t.tenantID)
}
