...
| Shipped-binary vulnerability scan | `go build -o ./aimux-vuln-scan ./cmd/aimux` then `govulncheck -mode=binary ./aimux-vuln-scan` | Non-zero exit; source-mode findings remain visible in the soft-fail Security workflow for dependency hygiene |
